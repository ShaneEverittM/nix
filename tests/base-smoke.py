"""Behavioral smoke test for the shared NixOS base modules.

``tests/base-smoke.nix`` substitutes the public identity values, then passes the
NixOS test driver's dynamic objects to ``run`` at its sole runtime boundary.
"""
...
USER_EMAIL = "@identityUserEmail@"


def run(machine: QemuMachine, subtest: Subtest) -> None:
    ...
    with subtest("only publickey auth is offered on the wire"):
        ...
        _ = machine.succeed("command -v ssh")
        ssh_command = " ".join(
            [
                "ssh",
        ...
        denied = machine.fail(ssh_command)
    ...
    with subtest("discovery, DNS, and tailnet daemons are up"):
        ...
        _ = machine.succeed("resolvectl status >/dev/null")
    ...
    with subtest("memory backstops are armed"):
        ...
        _ = machine.succeed("swapon --show=NAME --noheadings | grep -q zram")
    ...
    with subtest("home-manager fold-in produced the shared identity"):
        ...
        email = machine.succeed(f"su - {USERNAME} -c 'git config --get user.email'").strip()
        assert email == USER_EMAIL, f"git user.email is {email!r}"
        _ = machine.succeed(f"getent passwd {USERNAME} | grep -q zsh")
        _ = machine.succeed("nh --version")
