# Boots the real vaulthunters module in a VM and smoke-tests the console *plumbing*:
...
#
# What it does NOT test: the actual Forge/Vault Hunters server. That needs a multi-GB,
...
pkgs.testers.runNixOSTest {
  name = "vaulthunters-console";

  nodes.machine =
    { ... }:
    {
      imports = [ ../hosts/exodus/vaulthunters.nix ];
      ...
      # Swap the Forge launcher (needs the modpack) for a stand-in that keeps the exact
      ...
      systemd.user.services.vaulthunters.serviceConfig = {
        ExecStartPre = lib.mkForce [ ];
        ExecStart = lib.mkForce (
          pkgs.writeShellScript "vh-console-stub" ''
            echo "vh-console-stub up"
  ...
  testScript = ''
    ...
    machine.wait_until_succeeds(as_shane("systemctl --user is-active vaulthunters.socket"))
    machine.wait_for_file("/run/user/1000/vaulthunters.stdin")
    machine.wait_until_succeeds(as_shane("systemctl --user is-active vaulthunters.service"))
    ...
    machine.succeed("echo list > /run/user/1000/vaulthunters.stdin")
    ...
    machine.succeed("echo 'say hi' > /run/user/1000/vaulthunters.stdin")
    ...
    machine.succeed(as_shane("systemctl --user is-active vaulthunters.service"))
  '';
}
